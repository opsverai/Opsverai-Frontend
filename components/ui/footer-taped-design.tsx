...
import { Facebook, Linkedin, Mail, X, Youtube } from "lucide-react";
import { useLocation } from "react-router-dom";

import { Wordmark } from "@/components/brand";
import { COMPANY, companyLinkedInUrl, companyMailto, companyMapsUrl } from "@/lib/company";
import { cn } from "@/lib/utils";

/** Pinterest-style “P” (not in lucide-react); slightly heavy stroke reads at small sizes. */
function PinterestIcon({ className, strokeWidth = 2, ...props }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      className={cn("h-4 w-4 shrink-0 sm:h-[1.125rem] sm:w-[1.125rem]", className)}
      aria-hidden
      {...props}
    >
      <path
        d="M11 20V5h4q4 4 0 8h-4"
        stroke="currentColor"
...
type SocialIconProps = { className?: string; strokeWidth?: number; "aria-hidden"?: boolean };

const socialLinks: readonly {
  label: string;
  href: string;
  Icon: ComponentType<SocialIconProps> | typeof PinterestIcon;
  /** Dock tile hover — brand-tinted, still editorial on dark UI */
  dockHover: string;
}[] = [
  {
    label: "X",
    href: "https://x.com/Opsverai",
    Icon: X,
    dockHover: "hover:bg-white/[0.08] hover:text-white",
  },
  {
    label: "Facebook",
    href: "https://web.facebook.com/Opsverai",
    Icon: Facebook,
    dockHover: "hover:bg-[#1877F2]/18 hover:text-[#9ec7ff]",
  },
  {
    label: "LinkedIn",
    href: companyLinkedInUrl,
    Icon: Linkedin,
    dockHover: "hover:bg-[#0A66C2]/18 hover:text-[#8ec5ff]",
  },
  {
    label: "Pinterest",
    href: "https://www.pinterest.com/Opsverai/",
    Icon: PinterestIcon,
    dockHover: "hover:bg-[#E60023]/16 hover:text-[#ff8a9a]",
  },
  {
    label: "YouTube",
    href: "https://www.youtube.com/@Opsverai",
    Icon: Youtube,
    dockHover: "hover:bg-[#ff0000]/14 hover:text-[#ff9b9b]",
  },
];
...
/** Connect links for quick product, call, and directions actions. */
const connectLinks = [
  { label: "Start now", href: "/product" },
  { label: "Office & directions", href: companyMapsUrl, external: true },
] as const;

export type FooterTapedDesignProps = {
  className?: string;
};

/**
 * Taped “sticky note” footer card — Vite/React (anchor links, not Next.js).
 * Uses shadcn semantic colors so it respects light/dark theme.
 */
export function FooterTapedDesign({ className }: FooterTapedDesignProps) {
  ...
  return (
    <footer className={cn("mx-auto w-full max-w-7xl px-4 text-foreground", className)}>
      <div className="relative mx-auto flex w-full max-w-7xl flex-col items-center gap-6 rounded-3xl border border-border/60 bg-card px-4 py-10 shadow-[0_24px_60px_-24px_rgb(0_0_0/0.35)] md:flex-row md:justify-between">
        <div className="absolute -top-4 -left-8 hidden h-[36px] w-[80px] scale-75 md:block" aria-hidden>
          {tape}
        </div>
        <div className="absolute -top-4 -right-8 hidden h-[36px] w-[80px] scale-75 rotate-90 md:block" aria-hidden>
          {tape}
        </div>

        <div className="flex w-full flex-1 flex-col items-start justify-between gap-6 px-2 md:flex-row md:gap-10 md:px-8">
          <div className="flex flex-col items-start gap-2">
            <a
              href={resolveSectionHref("#home")}
              className="flex flex-row items-center justify-start gap-1 text-2xl font-extrabold font-display text-foreground"
              aria-label={`${COMPANY.legalName} home`}
            >
              <Wordmark className="h-12 w-auto object-contain sm:h-14 md:h-16" />
            </a>
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Idea-to-startup platform</p>
            <p className="w-full text-base font-medium text-muted-foreground md:w-4/5">
              AI-powered innovation workspace that structures ideas into business models, feature plans, positioning, and execution-ready
              blueprints.
            </p>
            <a
              href={companyMailto}
              className="mt-1 inline-flex items-center gap-2 text-sm font-medium text-foreground/90 transition-colors hover:text-primary"
            >
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-border/60 bg-background/50 text-muted-foreground">
          ...
          <div className="flex w-full flex-col items-start gap-8 md:mx-4 md:flex-row md:gap-16 lg:gap-20">
            <div className="flex flex-col gap-2 md:gap-4">
              <h4 className="font-display text-sm font-semibold text-muted-foreground uppercase">Product</h4>
            ...
            <div className="flex flex-col gap-2 md:gap-4">
              <h4 className="font-display text-sm font-semibold text-muted-foreground uppercase">Company</h4>
              <div className="flex flex-wrap items-start gap-2 text-sm md:flex-col">
            ...
            <div className="flex flex-col gap-2 md:gap-4">
              <h4 className="font-display text-sm font-semibold text-muted-foreground uppercase">Connect</h4>
              <div className="flex flex-col items-start gap-2 text-sm">
                {connectLinks.map((link) => (
                  <a
                    key={link.label}
                    href={link.href}
                    {...("ariaLabel" in link && link.ariaLabel ? { "aria-label": link.ariaLabel } : {})}
                    {...(link.external
                      ? { target: "_blank", rel: "nofollow noopener noreferrer" }
                      : {})}
                    onClick={(event) => {
                      if (!link.href.startsWith("tel:")) return;
                      // Some desktop browsers ignore tel anchors unless explicitly navigated.
      ...
      <div className="mt-6 flex flex-col items-start justify-between gap-4 px-4 text-sm text-muted-foreground md:flex-row md:items-center md:px-8">
        <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-8">
          <p className="whitespace-nowrap text-foreground/90">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2.5 sm:w-auto sm:justify-end" aria-label="Social media">
          {socialLinks.map(({ label, href, Icon, dockHover }, i) => (
            <a
              key={label}
              href={href}
              target="_blank"
              rel="nofollow noopener noreferrer"
              aria-label={`${COMPANY.legalName} on ${label}`}
              className={cn(
                "group relative flex h-11 w-11 items-center justify-center sm:h-[3.15rem] sm:w-[3.15rem]",
                "rounded-2xl border border-border/45",
                "bg-gradient-to-b from-card/85 via-card/55 to-card/35 backdrop-blur-lg",
                "shadow-[0_10px_32px_-14px_rgb(0_0_0/0.55)] ring-1 ring-inset ring-white/[0.07] dark:ring-white/[0.05]",
                "text-muted-foreground transition-[color,background-color,transform,box-shadow,border-color] duration-300 ease-out",
                "hover:z-[1] hover:-translate-y-1.5 hover:rotate-0 hover:shadow-[0_18px_40px_-12px_rgb(0_0_0/0.55)] motion-reduce:hover:translate-y-0",
                "focus-visible:z-[1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55 focus-visible:ring-offset-2 focus-visible:ring-offset-background",
                i % 2 === 0
                  ? "motion-safe:sm:-rotate-[4deg] motion-safe:sm:-translate-y-0.5 motion-reduce:sm:rotate-0 motion-reduce:sm:translate-y-0"
                  : "motion-safe:sm:rotate-[4deg] motion-safe:sm:translate-y-1 motion-reduce:sm:rotate-0 motion-reduce:sm:translate-y-0",
                dockHover,
              )}
            >
              <span
                className="pointer-events-none absolute inset-x-2.5 top-[5px] h-px rounded-full bg-gradient-to-r from-transparent via-primary/35 to-transparent opacity-70"
                aria-hidden
              />
              <span
                className="pointer-events-none absolute -bottom-px left-1/2 h-4 w-[55%] -translate-x-1/2 rounded-[100%] bg-primary/10 opacity-0 blur-md transition-opacity duration-300 group-hover:opacity-100"
                aria-hidden
              />
              <Icon
                className="relative z-[1] h-[1.05rem] w-[1.05rem] shrink-0 opacity-88 transition-[opacity,transform] duration-300 group-hover:scale-110 group-hover:opacity-100 motion-reduce:group-hover:scale-100 sm:h-[1.2rem] sm:w-[1.2rem]"
                strokeWidth={label === "Pinterest" ? 2.1 : 1.65}
                aria-hidden
              />
            </a>
          ))}
